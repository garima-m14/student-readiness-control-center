...
if (existsSync(env)) {
  ...
  const lines = [
    `DATABASE_URL=postgresql://readiness:${dbPassword}@localhost:5432/readiness`,
    "MONGODB_URL=mongodb://localhost:27018/readiness",
    "MONGO_PORT=27018",
  ...
  console.log(
    "Created local credentials in .env. Use SEED_PASSWORD to sign in.",
  );
}
if (!existsSync(new URL("backend/.env", root)))
  copyFileSync(
    fileURLToPath(env),
    fileURLToPath(new URL("backend/.env", root)),
  );
