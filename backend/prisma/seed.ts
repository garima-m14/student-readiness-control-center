...
export async function seed() {
  ...
  if (!password || password.length < 12)
    throw new Error("SEED_PASSWORD must have at least 12 characters");
  ...
  for (const name of ["Acme Training", "Northstar Academy"]) {
    ...
    const users: User[] = [];
    for (const role of ["ADMIN", "EVALUATOR", "VIEWER"] as const)
      users.push(
        await db.user.upsert({
          ...
          create: {
            tenantId: tenant.id,
            name: `${slug === "acme" ? "Alex Morgan" : "Jordan Lee"} · ${role.toLowerCase()}`,
            email: `${role.toLowerCase()}@${slug}.test`,
            passwordHash,
            role,
          },
    ...
    for (const [index, studentName] of names.entries()) {
      const email =
        studentName.toLowerCase().replace(" ", ".") + `@${slug}.students.test`;
      if (
      ...
      await db.$transaction(async (tx) => {
        const student = await tx.student.create({
          data: { tenantId: tenant.id, name: studentName, email, phone: null },
        });
            ...
            await tx.attempt.create({
              data: {
                ...
                attemptedAt: new Date(
                  `2026-09-${round === 0 ? "12" : "20"}T10:00:00Z`,
                ),
              },
            });
        }
        ...
        await tx.attempt.create({
          data: {
            ...
            score: target,
            evaluatorId: users[1].id,
            attemptedAt: new Date("2026-09-20T10:00:00Z"),
          },
        });
        await tx.attempt.create({
          data: {
            ...
            score: 0,
            evaluatorId: users[1].id,
            attemptedAt: new Date("2026-09-21T10:00:00Z"),
            voidedAt: new Date(),
          },
        });
        const evidence = await tx.attempt.findMany({
          where: { tenantId: tenant.id, studentId: student.id },
        });
        const readiness = calculateReadiness(
          evidence.map((a) => ({ ...a, score: Number(a.score) })),
        );
        await tx.student.update({
          where: { tenantId_id: { tenantId: tenant.id, id: student.id } },
          data: { score: readiness.score, status: readiness.status },
        });
      });
    }
  }
