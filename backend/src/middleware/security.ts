...
export function authenticated(req: Request, res: Response, next: NextFunction) {
  authenticate(String(req.cookies?.session || ""))
    .then((user) => {
      req.identity = user;
      next();
    })
...
export function writers(req: Request, _res: Response, next: NextFunction) {
  next(
    req.identity.role === "VIEWER"
      ? new ApiError(
          403,
          "FORBIDDEN",
...
export function sameOrigin(req: Request, _res: Response, next: NextFunction) {
  if (
    ...
    (req.get("X-Requested-With") !== "readiness" ||
      (req.get("Origin") && req.get("Origin") !== process.env.FRONTEND_ORIGIN))
  )
    return next(
