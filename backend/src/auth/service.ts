...
export async function login(input: z.infer<typeof loginSchema>) {
  ...
  const session = await db.session.create({
    data: { userId: user.id, expiresAt: new Date(Date.now() + 8 * 3600_000) },
  });
  return {
    token: jwt.sign({ sid: session.id }, secret(), {
      subject: user.id,
      expiresIn: "8h",
      issuer: "readiness",
      audience: "readiness-web",
      algorithm: "HS256",
    }),
    user: identity(user),
...
export const identity = (user: IdentitySource) => ({
  id: user.id,
...
export async function authenticate(token: string): Promise<Identity> {
  ...
  try {
    payload = z
      .object({ sub: z.string().uuid(), sid: z.string().uuid() })
      .parse(
