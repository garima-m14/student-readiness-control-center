...
export const mutationLimit = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req) => req.identity.id,
  handler: (req, res) =>
    res
      .status(429)
      .json({
...
students.get(
  ...
  asyncHandler(async (req, res) =>
    res.json(
      await listStudents(req.identity.tenantId, listSchema.parse(req.query)),
    ),
  ),
);
students.get(
  ...
  asyncHandler(async (req, res) =>
    res.json(
      await detail(req.identity.tenantId, idSchema.parse(req.params.id)),
    ),
  ),
);
students.patch(
...
students.get(
  "/:id/activity",
  asyncHandler(async (req, res) => {
    const q = activitySchema.parse(req.query);
    res.json(
      await activity(
        req.identity.tenantId,
        idSchema.parse(req.params.id),
