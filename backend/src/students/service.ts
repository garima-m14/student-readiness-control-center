...
export async function listStudents(
  ...
  if (
    ...
    throw new ApiError(
      400,
      "VALIDATION_ERROR",
      "Cursor is no longer available; refresh the list",
    );
  const [field, direction] = q.sort.split(":") as [
    "name" | "email" | "score" | "status" | "createdAt",
    "asc" | "desc",
  ];
  ...
  return {
    items,
    total,
    nextCursor: more ? `${payload}.${signature(payload)}` : null,
    summary: Object.fromEntries(
      groups.map((g) => {
        if (typeof g._count !== "object" || g._count._all === undefined)
          throw new Error("Student count projection is missing");
        return [g.status, g._count._all];
      }),
    ),
  };
}
export async function updateStudent(
  ...
  return db.$transaction(async (tx) => {
    const result = await tx.student.updateMany({
      where: { tenantId, id, version: input.expectedVersion },
      data: { name: input.name, phone: input.phone, version: { increment: 1 } },
    });
    if (!result.count) {
      if (
