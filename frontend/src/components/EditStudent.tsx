...
export function EditStudent({ student }: { student: Student }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    setSaved(false);
    try {
      await api(`/students/${student.id}`, detailSchema, {
        method: "PATCH",
        body: JSON.stringify({
          name: data.get("name"),
      ...
      if (err instanceof ApiError && err.code === "CONFLICT")
        await client.invalidateQueries();
      setError(err instanceof Error ? err : new Error("Unable to save"));
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  return (
    <details className="panel edit-panel">
      <summary>
        Edit student information <span>Version {student.version}</span>
      </summary>
